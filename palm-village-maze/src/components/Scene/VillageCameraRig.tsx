...
interface VillageCameraRigProps {
...
export default function VillageCameraRig({
  cameraMode,
  characterPosRef,
  characterSpeedRef,
  cameraYawRef,
  ...
  useFrame((_, delta) => {
    ...
    if (cameraMode === 'pov') {
      ...
      // Scenic village alleyway angle
      _anchorTarget.set(charPos.x, 1.05, charPos.z - 0.35);
      _desiredCam.set(charPos.x + 0.22, 1.62, charPos.z + 2.85);
      clampCameraToCorridor(_anchorTarget, _desiredCam, 0.28, _safeCam);

      _lookTarget.set(charPos.x - 0.08, 1.42, charPos.z - 3.2);
      ...
      // 3rd-person follow cam with wall-collision spring arm
