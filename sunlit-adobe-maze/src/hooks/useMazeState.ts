...
> = {
  follow: {
    ...
    description: 'Smooth corridor follow camera behind Mina with wall-aware spring arm',
  },
  pov: {
    label: '👁️ 1st-Person Character POV',
    shortLabel: 'Character POV (Eyes)',
    description: 'See directly through Mina’s eyes with natural walking head-bob',
  },
  painting: {
    label: '🖼️ Watercolor Archway Angle',
    shortLabel: 'Painting Angle',
    description: 'Architectural gallery composition matching the reference artwork',
  },
};
...
export function useMazeState() {
  ...
  const [targetZoneId, setTargetZoneId] = useState<string>('golden-sanctuary');

  const characterPosRef = useRef<THREE.Vector3>(
    new THREE.Vector3(START_CELL_WORLD.x + 0.15, 0, START_CELL_WORLD.z + 1.15)
  );
  const characterYawRef = useRef<number>(Math.PI); // Facing North (-Z)
  ...
  const waveTimerRef = useRef<number>(1.8); // Cheerful greeting wave on load!
